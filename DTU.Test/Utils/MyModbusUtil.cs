...
namespace DTU.Test.Utils
{
    public class MyModbusUtil
    {
        ...
        /// <returns></returns>
        public static byte[] GetReadCmd(int slaveStation, ReadType readType, ushort startAdr, ushort length)
        {
            //定义临时字节列表
        ...
        /// <returns></returns>
        public static byte[] GetData(byte[] receiveMsg, int length, MyModbusUtil.ReadType readType)
        {
            ...
            if (receiveMsg.Skip(1).Take(1).ToArray()[0] != (byte)readType)
            {
